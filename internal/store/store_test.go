...
func TestOpen_CreatesSchema(t *testing.T) {
	...
	for _, table := range []string{"vault_fields", "vault_access_log", "vault_tokens", "vault_meta", "vault_attachments"} {
		var name string
...
func TestCheckpoint_FlushesWAL(t *testing.T) {
	db := tmpDB(t)
	...
	// Data must still be readable after checkpoint.
	v, err := db.GetMeta("k")
	if err != nil {
		t.Fatal(err)
	}
	if v != "v" {
		t.Fatalf("expected value after checkpoint, got %q", v)
	}
}
