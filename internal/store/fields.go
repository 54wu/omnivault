...
// UpdateFieldCipher rewrites a field's ciphertext without bumping its version.
// Used when re-encrypting all fields during a password change.
func (d *DB) UpdateFieldCipher(id, ciphertext string, updatedAt time.Time) error {
	_, err := d.conn.Exec(
		`UPDATE vault_fields SET value = ?, updated_at = ? WHERE id = ?`,
		ciphertext, updatedAt.UTC().Format(time.RFC3339), id,
	)
	return err
}
