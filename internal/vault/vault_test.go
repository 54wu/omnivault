...
func TestChangePassword_Roundtrip(t *testing.T) {
	...
	newToken, err := v.ChangePassword(testPassword, newPassword)
	if err != nil {
		t.Fatalf("ChangePassword failed: %v", err)
	}
	if newToken == "" {
		t.Fatal("expected non-empty new session token")
	}

	// Data must survive re-encryption.
	f, err := v.Get("identity.full_name")
	if err != nil {
		t.Fatalf("Get after ChangePassword: %v", err)
	}
	if f == nil || f.Value != "Jane Smith" {
		t.Fatalf("data lost after ChangePassword: %+v", f)
	}

	// Old session token must be invalid; the new one must work.
	dir := v.dir
	v.Lock()

	if _, err := v.Unlock(testPassword, sk); err != ErrWrongPassword {
		t.Fatalf("expected old password to fail after change, got %v", err)
	}
	tok2, err := v.Unlock(newPassword, sk)
	if err != nil {
		t.Fatalf("expected new password to unlock, got %v", err)
	}
	if tok2 == "" {
		t.Fatal("expected non-empty token after re-unlock")
	}

	// Reopening from disk still works with the new password.
	v.Close()
	v2, err := Open(dir)
	if err != nil {
		t.Fatal(err)
	...
	defer v2.Close()
	if _, err := v2.Unlock(newPassword, sk); err != nil {
		t.Fatalf("reopen with new password failed: %v", err)
	}
	f2, err := v2.Get("identity.full_name")
	if err != nil || f2 == nil || f2.Value != "Jane Smith" {
		t.Fatalf("data lost after reopen: %+v err=%v", f2, err)
	}
}

func TestChangePassword_WrongOldPassword(t *testing.T) {
	v, _ := tmpVault(t)
	if _, err := v.ChangePassword("wrong-old-password", "whatever-12345"); err != ErrWrongPassword {
		t.Fatalf("expected ErrWrongPassword, got %v", err)
	}
}
...
func TestChangePassword_ShortNewPassword(t *testing.T) {
	v, _ := tmpVault(t)
	if _, err := v.ChangePassword(testPassword, "short"); err == nil {
