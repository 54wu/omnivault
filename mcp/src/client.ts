...
function vaultDir(): string {
  return process.env.VAULT_DIR ?? join(homedir(), ".omnivault");
}
...
export async function mergePlan(
  source: unknown,
  person?: string,
): Promise<{ plan: unknown; unmatched: string[] }> {
  const q = person ? `?person=${encodeURIComponent(person)}` : "";
  return apiRequest("POST", `/vault/merge/plan${q}`, source) as Promise<{
    plan: unknown;
    unmatched: string[];
  }>;
}
...
export async function mergeApply(
  plan: unknown,
): Promise<{ status: string; applied: number; kept: number; skipped: number }> {
  return apiRequest("POST", "/vault/merge/apply", plan) as Promise<{
    status: string;
    applied: number;
    kept: number;
    skipped: number;
