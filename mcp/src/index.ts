...
import { z } from "zod";
import { getStatus, getSchema, getField, listFields, getContext, setField, mergePlan, mergeApply, provisionServiceToken } from "./client.js";
...
const mergeItemSchema = z.object({
  label: z.string().describe("材料条目的中文标签，如 姓名、硕士院校"),
  value: z.string().describe("材料条目的值"),
});
...
server.registerTool(
  ...
  {
    description:
      "Three-tier merge dry-run: classify a material source against a person's dossier and produce a decision plan WITHOUT writing anything. Tiers: auto (no-op or safe fill), batch (conflicts on low-sensitivity fields), manual (sensitive/critical). If 'person' is omitted, ownership is auto-verified from person_hint. Returns the plan, which you should review and submit to the user for decisions on batch/manual items before applying.",
    inputSchema: {
      material: z
        .object({
          source: z.string().optional().describe("材料来源标识，如 example.com"),
          person_hint: z
            .object({
            ...
            .optional()
            .describe("用于归属校验的身份信号"),
          items: z.array(mergeItemSchema).describe("材料条目列表"),
        })
        .describe("材料：label->value 条目列表，可选归属提示"),
      person: z
        .string()
        .optional()
        .describe("目标人员 id，如 p1。缺省时按 person_hint 自动归属（可能返回待决）"),
    },
  },
  async ({ material, person }): Promise<CallToolResult> => {
    try {
      const res = await mergePlan(material, person);
...
server.registerTool(
  ...
  {
    description:
      "Apply a merge decision plan. Writes only the items whose action is fill/replace/add; keep/skip are ignored. Use this AFTER the user has decided the batch/manual items from vault_merge_plan. Always confirm with the user before applying. Writes are backed up and audited automatically.",
    inputSchema: {
      plan: z
        .any()
        .describe("The decision plan returned by vault_merge_plan (with actions filled in by the user)"),
    },
  },
  async ({ plan }): Promise<CallToolResult> => {
    try {
      const res = await mergeApply(plan);
      return ok(JSON.stringify(res, null, 2));
    } catch (e) {
      return err((e as Error).message);
    }
